...
@implementation CLDrawTool
{
    ...
    CGPoint _prevprevDraggingPosition;
}
...
- (void)drawingViewDidPan:(UIPanGestureRecognizer*)sender
{
    ...
    if(sender.state == UIGestureRecognizerStateBegan){
        _prevprevDraggingPosition = currentDraggingPosition;
        _prevDraggingPosition = currentDraggingPosition;
    }
    
    if(sender.state != UIGestureRecognizerStateEnded){
        
        [self drawLine:_prevDraggingPosition to:currentDraggingPosition pre:_prevprevDraggingPosition];
    }
    
    _prevprevDraggingPosition = _prevDraggingPosition;
    _prevDraggingPosition = currentDraggingPosition;
}

-(void)drawLine:(CGPoint)from to:(CGPoint)to pre:(CGPoint)pre
{
    ...
    CGPoint mid1 = [self calculateMidPointForPoint:from andPoint:pre];
    CGPoint mid2 = [self calculateMidPointForPoint:to andPoint:from];
    
    CGContextMoveToPoint(context, mid1.x, mid1.y);
    // Use QuadCurve is the key
    CGContextAddQuadCurveToPoint(context, from.x, from.y, mid2.x, mid2.y);
